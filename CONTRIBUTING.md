...
1. **Run tests**: `just test` (or `uv run pytest`)
1. **Lint/Format**: `just check` / `just fix`
...
1. Add a generic wrapper in `src/bounty_core/gog.py` (optional, but follows pattern).
1. Update `src/bounty_discord/utils.py:resolve_game_details` to handle the new store logic.
