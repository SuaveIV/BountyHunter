...
import aiohttp
from bounty_core.fetcher import fetch_bsky_feed, extract_links_from_text
from bounty_core.parser import parse_post_for_links
from bounty_core.store import Store
from bounty_core.steam import fetch_game_details
from .config import BOT_TOKEN, DATABASE_PATH, POLL_INTERVAL, ADMIN_DISCORD_ID
from .logging_config import get_logger

logger = get_logger(__name__)

class Bounty(Extension):
    ...
    async def on_load(self):
        ...
        logger.info("Bounty extension loaded")
    ...
    async def _send_admin_dm(self, message: str):
        try:
            if ADMIN_DISCORD_ID:
                ...
                if admin_user:
                    await admin_user.send(f"Bounty Error: {message}")
    ...
    async def subscribe_command(self, ctx: PrefixedContext):
        try:
            ...
            await ctx.send("This channel is subscribed to bounty announcements.")
        except Exception as e:
