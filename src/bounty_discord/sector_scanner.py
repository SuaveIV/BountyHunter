...
class SectorScanner:
    ...
    async def scan(self, limit: int = 10, ignore_seen: bool = False) -> list[tuple[str, dict[str, Any]]]:
        ...
        NOTE: Valid posts are NOT marked as seen here. The caller (SectorVisor._announce_new)
        is responsible for marking each post seen after it has been announced. This prevents
        posts from being silently lost if the bot crashes or a send fails before announcement.

        INFO posts are still marked seen immediately since they are intentionally discarded
        and should never be re-processed.
        """
        try:
            ...
            for post in posts:
                ...
                if content_type == "INFO":
                    ...
                    # Mark INFO posts seen immediately — they are intentionally discarded,
                    # not failed, so there is nothing to retry.
                    if not ignore_seen:
                ...
                if valid_links:
                    ...
                    # BUG FIX: Previously marked posts as seen here, before _announce_new
                    # was called. A crash or send failure would silently lose the post.
                    # Posts are now marked seen by SectorVisor._announce_new after
                    # announcement is attempted.
